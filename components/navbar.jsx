    ...
    useEffect(() => {
        if (isMounted) {
            ...
            if (darkMode && button_ball.current) {
                button_ball.current.style.left = "auto"; 
                button_ball.current.style.right = "5px";
            } else if (!darkMode && button_ball.current) {
                button_ball.current.style.right = "auto"; 
                button_ball.current.style.left = "5px";
            }
        }
    }, [darkMode, isMounted]);
    ...
    return (
        <>
            <div className='w-[650px] h-[75px] bg-light-cardBackground dark:bg-dark-cardBackground rounded-[10px] flex items-center justify-evenly pl-[10px] pr-[10px] absolute left-[50%] top-[20px] translate-x-[-50%] z-[2]'>
                ...
                <Link href="#" className='font-inter-regular text-light-primaryText dark:text-dark-primaryText text-[20px] font-medium opacity-[.8] hover:opacity-[1]'>Services</Link>
                <Link href="#" className='font-inter-regular text-light-primaryText dark:text-dark-primaryText text-[20px] font-medium opacity-[.8] hover:opacity-[1]'>Works</Link>
                <Link href="#" className='font-inter-regular text-light-primaryText dark:text-dark-primaryText text-[20px] font-medium opacity-[.8] hover:opacity-[1]'>About</Link>
                <Link href="#" className='font-inter-regular text-light-primaryText dark:text-dark-primaryText text-[20px] font-medium opacity-[.8] hover:opacity-[1]'>Articles</Link>
                <Link href="#" className='font-inter-regular text-light-primaryText dark:text-dark-primaryText text-[20px] font-medium opacity-[.8] hover:opacity-[1]'>Contact</Link>
                <div className='w-[60px] h-[30px] cursor-pointer bg-dark-background dark:bg-light-background rounded-[50px] relative border border-light-background dark:border-dark-background' onClick={mode_shift}>
                    <div className='absolute h-[20px] left-[5px] top-[50%] translate-y-[-50%] rounded-[50px] aspect-square bg-light-background dark:bg-dark-background transition' ref={button_ball}></div>
                </div>
            </div>
        </>
    )
